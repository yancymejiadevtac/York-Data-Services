...
//
var_deleted_value = IFNULL(row.Monthly_Service_Fee,0.00);
if(input.sc1 == true)
{
	var_deleted_value = IFNULL(row.sc_Total1,0.00);
}
// 
//
//GLOBAL VARIABLES
...
if(input.Add_another_service == true)
{
	...
	input.Service_Charges_Sub_Total_2 = monthly_fee2 - var_deleted_value;
	input.Service_Sub_Total_2 = sd_totals2 + IFNULL(input.Carriage_2,0) - var_deleted_value;
	/*
	*
	* Third Package
	*
...
//
tot_monthly = monthly_fee1 + monthly_fee2 + monthly_fee3 + monthly_fee4 - var_deleted_value;
