...
//
var_deleted_value = IFNULL(row.Fee,0.00);
if(input.sc3 == true)
{
	var_deleted_value = IFNULL(row.sf_Total3,0.00);
}
// 
//
//GLOBAL VARIABLES
	...
	if(input.Add_another_service_2 == true)
	{
		...
		*/
		if(input.Add_another_service_3)
		{
			...
			input.Up_Front_Sub_Total_4 = upfront_fee4 - var_deleted_value;
...
//
//OVERALL TOTALS
tot_upfront = upfront_fee1 + upfront_fee2 + upfront_fee3 + upfront_fee4 - var_deleted_value;
